...
public class DotSix extends Dot {
	...
	public LittleDot[] blast() {
		...
		for (LittleDot aLd : ld) {
			j = -j;
			aLd.x += (aLd.x - endPoint.x) / 4 + j;
			aLd.y += (aLd.y - endPoint.y) / 4 + 1;
		}
...
//			canvas.drawOval(oval, mPaint);
		...
		//added by Dumbbell Yang at 2014-05-03 ����������ʾ״̬
		else if (state == 5) {	
			//��ʾ�̻����� added by Dumbbell Yang at 2014-05-03
			...
			if (circle < maxCircle) {
				...
				for (LittleDot aLd : ld) {
					...
					canvas.drawLine(aLd.x, aLd.y, aLd.x
							+ (aLd.x - endPoint.x) / circle, aLd.y
							+ (aLd.y - endPoint.y) / circle, mPaint);
				}
			} else {
				for (LittleDot aLd : ld) {
					mPaint.setColor(aLd.color);
